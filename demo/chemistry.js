...
// ── CLIMATE RANGES ──
// NOTE: the `lsi` entries below are legacy/reference only. As of the surface-type
// feature, the LSI target band is surface-driven (see SURFACE_LSI + surfaceLSI()):
// the pool surface sets the ideal saturation balance, and climate applies only a
// modest shift on top (currently −0.1 in desert/arid, where evaporation lifts LSI).
// Climate primarily governs how fast chemistry drifts; it nudges the target band,
// but is not the main driver of it. R('lsi') no longer reads these CLIMATE_DATA.lsi values.
const CLIMATE_DATA = {
...
// ── POOL SURFACE ──
// Free feature (not Pro-gated): the pool's surface material adjusts the CH range
// and the LSI target band, layered on top of the selected climate.
//
// This app is LSI-forward: the primary surface signal is the LSI target band,
// not raw calcium. LSI (which already includes CH, pH, TA, temp, TDS) is what
// actually determines whether a surface is being protected or attacked.
//
// CH — cementitious surfaces (plaster, pebble) share the climate CH baseline;
//   they have the same calcium relationship, so their distinction lives entirely
//   in the LSI target below, not in a calcium floor. Inert surfaces (vinyl,
//   fiberglass) have no calcium demand, so their CH band shifts down to avoid
//   false "low CH" flags on pools that run lower calcium by design.
// LSI — surface-primary: the ideal saturation band is set by surface, not climate.
//   Desert gets a −0.1 shift because high evaporation naturally lifts LSI.
const SURFACE_DATA = {
  plaster:    { label:'Plaster / Gunite' },
  pebble:     { label:'Pebble / Quartz' },
  vinyl:      { label:'Vinyl' },
  fiberglass: { label:'Fiberglass' },
};

const SURFACE_LSI = {
  plaster:    { min:-0.2, max: 0.0 },
  pebble:     { min:-0.1, max: 0.1 },
  vinyl:      { min:-0.3, max:-0.1 },
  fiberglass: { min:-0.2, max: 0.0 },
};

// Surface is a free feature — no isPro() gate (unlike climate). Default plaster.
function getSurface() {
  return SURFACE_DATA[poolSettings.surface] ? poolSettings.surface : 'plaster';
}

// Round to 1 decimal to avoid float drift (e.g. -0.1 + -0.2 = -0.30000000000000004).
function round1(n) { return Math.round(n * 10) / 10; }

// CH range adjusted for surface, layered on the climate baseline. Cementitious
// surfaces (plaster, pebble) use the baseline unchanged — the surface difference
// is carried by the LSI target, not calcium. Inert surfaces shift their band down.
function surfaceCH(base) {
  switch (getSurface()) {
    case 'vinyl':      return { min: base.min - 100, max: base.max - 100 };
    case 'fiberglass': return { min: base.min - 50,  max: base.max - 50  };
    default:           return { min: base.min, max: base.max };  // plaster & pebble
  }
}

// LSI target band: surface-driven, with a modest −0.1 desert (evaporation) offset.
function surfaceLSI() {
  const band = SURFACE_LSI[getSurface()];
  const climateKey = isPro() ? (poolSettings.climate || 'temperate') : 'temperate';
  const shift = climateKey === 'desert' ? -0.1 : 0;
  return { min: round1(band.min + shift), max: round1(band.max + shift) };
}

function R(param) {
  if (param === 'ch')  return surfaceCH(getClimate().ch);
  if (param === 'lsi') return surfaceLSI();
  return getClimate()[param];
}

// ── LSI CALC ──
// Standard pool Langelier Saturation Index (APSP/Taylor factor convention).

// Temp Factor: quadratic least-squares fit to the APSP/Taylor LSI temperature-factor
// table (TF rises from 0.0 @32°F to 0.9 @103°F; e.g. 60→0.4, 76→0.6, 84→0.7, 94→0.8).
// Continuous — no stepped jumps at bucket boundaries. Max error vs the published table
// points is ~0.02; the table is itself a stepped, industry-standard approximation.
function tempFactor(tempF) {
  return -0.00004265867 * tempF * tempF + 0.01841027 * tempF - 0.5471549;
}

// LSI = pH + TempFactor + TDSFactor + CalciumFactor + AlkFactor − 12.1
// TempFactor    = quadratic fit to APSP/Taylor temperature-factor table (°F)
// TDSFactor     = 0.012 × log10(salt_ppm)
// CalciumFactor = log10(CH) − 0.4   (standard Langelier calcium-hardness factor;
//                 the 0.4 is subtracted after the log, not from CH before it)
...
function computeLSI(ph, tempF, ta, ch, cya, salt) {
  ...
  const caF  = (ch > 0)        ? Math.log10(ch) - 0.4     : 0;
...
function healthScoreColor(score) {
  ...
  if (score >= 65) return 'var(--caution)';
